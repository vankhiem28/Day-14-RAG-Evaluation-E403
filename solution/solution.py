...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial judge. Score the AI response on each criterion "
            "from 0.0 to 1.0 and return ONLY a JSON object mapping criterion "
            "names to scores.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{criteria_lines}\n\n"
            "Return JSON like: {\"criterion1\": 0.8, \"criterion2\": 0.7}"
        )
        ...
        try:
            ...
            default_scores = {key: 0.5 for key in rubric}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in scores_batch:
            ...
            if not scores:
                continue
            flat = [float(v) for v in scores.values() if isinstance(v, (int, float))]
            if not flat:
                continue
            all_scores.extend(flat)
            first_scores.append(flat[0])

        positional_bias = False
        if len(first_scores) >= 2:
            first_higher = sum(
                1 for s in first_scores if s > 0.5
            ) / len(first_scores)
            positional_bias = first_higher >= 0.7
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        for r in results:
            if r.passed or r.failure_type is None:
                continue
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, failure in enumerate(failures, start=1):
            ...
            fix = (
                ...
                else "Review pipeline and re-evaluate"
            )
    ...
    def generate_improvement_suggestions(
        ...
        type_to_suggestion = {
            "hallucination": "Implement a hallucination guardrail: filter any answer tokens not present in the retrieved context before responding.",
            "irrelevant": "Add explicit intent detection and rephrase ambiguous user questions before retrieval to improve routing.",
            "incomplete": "Increase chunk size or retrieve more top-k chunks to reduce evidence fragmentation in the context window.",
            "off_topic": "Strengthen system prompt with explicit scope definition and add an out-of-scope refusal path.",
            "refusal": "Relax guardrails on legitimate queries and add an allow-list of in-scope topics.",
        }
        for ftype, _ in sorted(categories.items(), key=lambda x: -x[1]):
            if ftype in type_to_suggestion:
                suggestions.append(type_to_suggestion[ftype])

        if "hallucination" in categories:
            suggestions.append(
                "Add a faithfulness checker: cross-validate answer claims against retrieved context and downgrade unsupported statements."
            )
        if "incomplete" in categories:
            suggestions.append(
                "Add few-shot examples of complete answers to the prompt template to improve generation coverage."
            )
        if "irrelevant" in categories:
            suggestions.append(
                "Augment the retriever with query rewriting (HyDE) to better match user intent with relevant documents."
            )

        if len(suggestions) < 3:
            suggestions.append(
                "Re-run benchmark after each change to measure the impact of every fix against the golden dataset."
            )
            suggestions.append(
                "Cluster failures by root cause before fixing to address systemic issues rather than one-off cases."
            )
